...
STATIC CONST GPIO_IOMUX_CONFIG mSdmmc2IomuxConfig[] = {
  ...
  { "wifi_reg_on",        3, GPIO_PIN_PD4, 0, GPIO_PIN_PULL_NONE, GPIO_PIN_DRIVE_DEFAULT },
};
  ...
  )
{
  ...
  /* Set GPIO3 PD4 (WIFI_REG_ON) output high to enable WiFi */
  GpioPinSetDirection (3, GPIO_PIN_PD4, GPIO_PIN_OUTPUT);
  MicroSecondDelay (1000);
  GpioPinWrite (3, GPIO_PIN_PD4, FALSE);
  MicroSecondDelay (500000);
  GpioPinWrite (3, GPIO_PIN_PD4, TRUE);
  MicroSecondDelay (100000);
}
  ...
  )
{
  ...
  SocSetDomainVoltage (VCCIO1, VCC_3V3); // 40pin header
  // SocSetDomainVoltage (VCCIO2, VCC_1V8); // eMMC
  // SocSetDomainVoltage (VCCIO3, VCC_3V3); // SDMMC
  SocSetDomainVoltage (VCCIO4, VCC_1V8); // RTC
  SocSetDomainVoltage (VCCIO5, VCC_3V3); // 40pin header
  SocSetDomainVoltage (VCCIO6, VCC_1V8); // RGMII1_M1
  SocSetDomainVoltage (VCCIO7, VCC_3V3); // 40pin header
  ...
  /* WiFi setup */
  BoardInitWiFi (); // this is a phantom wifi module on SDIO
